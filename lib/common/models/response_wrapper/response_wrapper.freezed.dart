...
class _$ResponseWrapperImpl<T> implements _ResponseWrapper<T> {
  const _$ResponseWrapperImpl({this.status, this.message, required this.data});
  ...
  @override
  String toString() {
    return 'ResponseWrapper<$T>(status: $status, message: $message, data: $data)';
  }
...
abstract class _ResponseWrapper<T> implements ResponseWrapper<T> {
  const factory _ResponseWrapper(
      {final bool? status,
      final String? message,
      required final T? data}) = _$ResponseWrapperImpl<T>;
